...
export default function RecruiterDashboard({ token }) {
  ...
  const createJob = async () => {
    ...
    // Validate external URL if apply type is external
    if (form.apply_type === "external" && !form.external_apply_url.trim()) {
      showToast("Please provide the external apply URL", "error");
      return;
    }
    setCreating(true);
    const payload = {
  ...
  const updateStatus = async (appId, status) => {
    await apiFetch.patch(`/jobs/applications/${appId}/status/`, { status }, token);
        ...
        {showCreate && (
          <div className="card" style={{ padding: "24px", marginBottom: 20, borderLeft: "4px solid #e05c00" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              <div style={{ gridColumn: "1/-1" }}>
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
                  {[
                    ...
                    >
                      <div style={{ fontWeight: 700, fontSize: 13, color: form.apply_type === val ? "#e05c00" : "#1a1a1a" }}>
                        {title}
                      </div>
              ...
              {/* ── External URL — only shown when external is selected ── */}
                        ...
                        <tbody>
                          {candidates.map((c, idx) => (
                            <tr key={c.application_id || idx} style={{ borderTop: "1px solid #f3f2f1" }}>
                              ...
                              <td style={{ padding: "12px 16px", fontWeight: 600 }}>{c.candidate_name}</td>
